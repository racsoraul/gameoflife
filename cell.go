...
// GetCellPosFromWindowCoords Returns a cell's position from the given relative window's coordinates.
func (fb *FrameBuffer) GetCellPosFromWindowCoords(winX, winY int32) (int32, int32) {
	return winX / fb.g.cellSize, winY / fb.g.cellSize
}

// ToggleCellState Toggles a cell's state located at the window's coordinates in the current color buffer.
...
// GetCellNeighbourStates Get the state of the eight surrounding cells from the current color buffer.
